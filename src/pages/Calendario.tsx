...
function Calendario() {
    ...
    return (
      <div className="relative min-h-screen">
        <div
          ...
          style={{
            backgroundImage:
              "url('https://theenterpriseworld.com/wp-content/uploads/2025/07/2.-Ayrton-Senna-Source-history.co_.uk_.jpg')",
  ...
  const getSesionFecha = (tipo?: string) => {
    const s = carreraActual?.sessions
      ?.filter((s) =>
        tipo ? s.type === tipo : new Date(s.start_time) > new Date(),
      )
      .sort(
        (a, b) =>
          ...
          <Card className="bg-gradient-to-br from-primary/70 via-accent/70 to-primary/70">
            <CardHeader>
              <CardTitle className="text-3xl font-extrabold text-primary-foreground flex gap-3 border-b-2 border-accent pb-3">
                ...
                <Badge variant="secondary" className="text-sm">
                  {carreraActual
                    ? new Date(carreraActual.start_date).toLocaleDateString(
                        'es-ES',
                        {
                          day: 'numeric',
                        },
                      ...
                      ' - ' +
                      new Date(carreraActual.end_date).toLocaleDateString(
                        'es-ES',
                        {
                          day: 'numeric',
                          month: 'short',
            ...
            <CardContent>
              <div className="grid gap-3 max-h-80 overflow-y-auto">
                {carreraActual
                  ? carreraActual.sessions?.map((sesion) => {
                      return (
                        <div
                          key={sesion.id}
                        ...
                        >
                          <div className="flex items-center gap-3">
                            <Badge
                              variant={
                                sesion.type === 'GP' ? 'destructive' : 'default'
                              }
                            ...
                            <span className="font-semibold text-foreground">
                              {new Date(sesion.start_time).toLocaleDateString(
                                'es-ES',
                                {
                                  weekday: 'long',
                                  day: 'numeric',
                          ...
                          <span className="text-base font-mono font-bold text-foreground">
                            {new Date(sesion.start_time).toLocaleTimeString(
                              'es-ES',
                              {
                                hour: '2-digit',
                                minute: '2-digit',
